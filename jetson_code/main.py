...
url_string = 'http://119.70.16.37:3000/image/uploads/' 
...
URL_user = 'http://119.70.16.37:9002/api/user'
for a in range(len(requests.get(URL_user, headers = headers).json()['_embedded']['userResourceList'])):
...
def cam1():
    URL_recog = 'http://119.70.16.37:9002/api/reservations'
    user_name = 0
    car_id = 0
    global output_pin
    certification_name = ""
    ...
    # face_recognition
    while True:
        for i in range(len(requests.get(URL_recog, headers = headers).json()['_embedded']['reservationResourceList'])):
            print(f'num : {i}')
            recog = requests.get(URL_recog, headers = headers).json()['_embedded']['reservationResourceList'][i]['certification']
            print(f'recog : {recog}') # check recognition
            if(recog == 'DOING'):
                user_name = requests.get(URL_recog, headers = headers).json()['_embedded']['reservationResourceList'][i]['userId']
                car_id = requests.get(URL_recog, headers = headers).json()['_embedded']['reservationResourceList'][i]['reservationId']
                ...
                if user_name == certification_name: 
                    ...
                    URL_recog = 'http://119.70.16.37:9002/api/reservations/' + str(car_id)
                    print(URL_recog)
                    ...
                    break
        URL_recog = 'http://119.70.16.37:9002/api/reservations'
        if(requests.get(URL_recog, headers = headers).json()['_embedded']['reservationResourceList'][car_id-1]['certification'] != 'SUCCESS'):
            continue
        else:
            data = {'certification' : 'IDLE'}
            URL_recog = 'http://119.70.16.37:9002/api/reservations/' + str(car_id)
    ...
    while True:
        for i in range(len(requests.get(URL_recog, headers = headers).json()['_embedded']['reservationResourceList'])):
            otp_number = requests.get(URL_recog, headers = headers).json()['_embedded']['reservationResourceList'][i]['otp_number']
        ...
        URL_recog = 'http://119.70.16.37:9002/api/reservations'
        if (requests.get(URL_recog, headers = headers).json()['_embedded']['reservationResourceList'][car_id-1]['otp_number'] != 1):
            continue
        else: # otp reset
            data = {'otp_number' : '2'}
            URL_recog = 'http://119.70.16.37:9002/api/reservations/' + str(car_id)
...
def cam2():
  ...
  front_num=0
  URL_detection = 'http://119.70.16.37:9002/api/car/123호1234'
  # face_detection
      ...
      try :
        ...
        if data.decode(): # Start when receive data
              ...
              if data.decode() is not None:
                ...
                requests.put(URL_detection, json = db_data, headers = headers)
